...
VideoX.prototype.okSearch = function (id, title) {
  ...
  this.currentID = id;
  var url = "https://www.okzy.co/index.php?m=vod-search&wd=".concat(encodeURI(encodeURI(title)), "&submit=search");

  var imgUrlCb = function imgUrlCb(index, cb) {
...
VideoX.prototype.yjSearch = function (id, title) {
  ...
  this.currentID = id;
  var url = "http://yongjiuzy.cc/index.php?m=vod-search&wd=".concat(encodeURI(encodeURI(title)));
...
VideoX.prototype.mjSearch = function (id, title) {
  ...
  this.currentID = id;
  var url = "https://www.meiju.net/new/video/search/".concat(encodeURI(encodeURI(title)), ".html");
...
var searchReaultNode = function searchReaultNode() {
  /*<div class="vertical">
  <div class="link_wrap">
    <div class ="link">
...
function doCORSRequest(options, callback) {
  var updateProgress = arguments.length > 2 && arguments[2] !== undefined ? arguments[2] : null;
  ...
  x.setRequestHeader('Content-Type', 'application/x-www-form-urlencoded'); //   if (/^POST/i.test(options.method)) {
...
Menu.prototype.changeUI = function () {
  ...
  if (uiMgr.currentView == 'home' && uiMgr.hasShowSearchView || uiMgr.currentView == 'search' && uiMgr.hasShowPlayView) {
...
var webAppInit = function webAppInit() {
  menu.changeUI();
  uiMgr.showHomeView();
  ...
  document.getElementById('back_btn').onclick = function () {
    if (uiMgr.currentView == 'search') {
      ...
      play.pause();
      uiMgr.showSearchView();
    } else if (uiMgr.currentView == 'home') {// do nothing
    }
  };
  ...
  document.getElementById('front_btn').onclick = function () {
    ...
    } else if (uiMgr.currentView == 'play') {// do nothing
...
UIMgr.prototype.showSearchResultView = function (key, resultList, resParser, imgUrlCb) {
  ...
  if (document.getElementsByClassName(key).length <= 0) {
    for (var i = 0; i < resultList.length; i++) {
      ...
      document.getElementById("search_results_ul").appendChild(this.createSearchResultNode(i, key, title, href, resParser, imgUrlCb));
    }
  } else {
...
UIMgr.prototype.addCatalogView = function (key, title, resultList, resParser, imgUrlCb) {
  ...
  node.onclick = function () {
    _this4.showSearchResultView(key, resultList, resParser, imgUrlCb);
...
UIMgr.prototype.createSearchResultNode = function (index, key, title, href, resParser, imgUrlCb) {
  ...
  var li = document.createElement('li');
  li.setAttribute("class", "item_li ".concat(key));
  li.innerHTML = searchReaultNode.getMultiLine();
  li.getElementsByClassName("title_link")[0].innerHTML = title;
  imgUrlCb(index, function (url) {
    li.getElementsByClassName("div_img")[0].style.backgroundImage = "url(" + url + ")";
  });

  li.getElementsByClassName("div_img")[0].onclick = function () {
    uiMgr.showPlayView();
    uiMgr.clearSelectList();
    play.reset();
    dataMgr.requestData(href, function (responseText) {
      resParser.parserVideoPlaylist(responseText, function (playlist) {
        for (var i = 0; i < playlist.length; i++) {
          var title = playlist[i].title;
          var m3u8Url = playlist[i].m3u8Url;
          document.getElementById('select_ul').appendChild(_this5.createSelectNode(title, m3u8Url, resParser));
        }
        ...
        if (playlist.length > 0) {
          ...
          if (resParser.parserM3u8Url != null) {
            resParser.parserM3u8Url(m3u8Url, function (url) {
              if (url != null) {
                play.play(url);
              } else {
                alert('无效播放地址！');
...
DataMgr.prototype.requestData = function (url, callback) {
  doCORSRequest({
    method: "get",
    url: url // data: 'wd='+title+'&submit=search'
...
MJParser.prototype.parserM3u8Url = function (url, callback) {
  dataMgr.requestData(url, function (responseText) {
...
function fullScreenChange(isFullscreen) {
  try {
...
Player.prototype.reset = function () {
  if (this.player != null) {
    ...
    this.player.on('fullscreenchange', function (data) {
      fullScreenChange(data.detail); // outputObj(data);
...
function setFontSize() {
  ...
  de.style.fontSize = de.getBoundingClientRect().width / 20 + 'px'; // if (play.player != null){
